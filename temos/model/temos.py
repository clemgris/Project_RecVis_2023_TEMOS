...
class TEMOS(BaseModel):
    ...
    def text_to_motion_forward(self, text_sentences: List[str], lengths: List[int], *,
        ...
        output = self.motiondecoder(latent_vector, lengths)
        features = output[:,:,:64]
        contacts = self.sigmoid(output[:,:,64:])
        datastruct = self.Datastruct(features=features)

        if not return_latent:
            return datastruct
        return datastruct, contacts, latent_vector, distribution
    ...
    def allsplit_step(self, split: str, batch, batch_idx):
        ...
        ret = self.text_to_motion_forward(batch["text"],
                                          batch["length"],
                                          return_latent=True)
        datastruct_from_text, contacts_from_text, latent_from_text, distribution_from_text = ret

        # Encode the motion/decode to a motion
        ret = self.motion_to_motion_forward(batch["datastruct"],
                                            batch["length"],
                                            return_latent=True)
        datastruct_from_motion, contacts_from_motion, latent_from_motion, distribution_from_motion = ret
        ...
        loss = self.losses[split].update(ds_text=datastruct_from_text,
                                         ...
                                         contacts_motion = contacts_from_motion,
                                         contacts_text = contacts_from_text,
                                         contacts_ref = batch["contacts"], 
                                         velocities_ref = batch["velocities"])
