...
def regroup_metrics(metrics):
    ...
    if "foot_sliding" in dico:
        foot_sliding = dico.pop("foot_sliding")
        for name, metric_food_sliding in zip(mmm_joints, foot_sliding):
            dico[f"foot_sliding_{name}"] = metric_food_sliding
...
def evaluate(cfg: DictConfig) -> None:
    ...
    from temos.data.kit import load_mmm_keyid, load_amass_keyid, load_contact_keyid
    ...
    for keyid in keyids:
        ...
        if not amass:
            ...
            ref_contacts, _ = load_contact_keyid(keyid, contact_path)
        else:
        ...
        if not onesample:
            ...
            CMetrics_worst.update(model_joints_all, ref_joints_all, length_all, ref_contacts_all)
